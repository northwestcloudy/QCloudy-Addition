...
- [Changelog (current source 0.3.10-alpha1; public Beta 0.3.10 below)](CHANGELOG.md)
...
- **Player Profile Viewer** — local `//pv [player or UUID]` and `/qpv [player or UUID]` commands open a read-only QCA-styled SkyBlock profile browser; omitting the target selects the local player. The profile selector and draggable side/content scrollbars expose Overview, Gear, Accessories, Pets, Inventory/Ender Chest/Storage/Wardrobe/Vault, Skills, Slayer, Mining, Minions, Bestiary, Collections, Crimson Isle, Rift, Miscellaneous/Farming, Museum, Garden, and Market/Net Worth data. Ordinary `/pv` is deliberately not registered, and Dungeon/party-profile inspection is not part of this version. Market metadata distinguishes a complete snapshot from a partial/stale source, while an item with no defensible Bazaar, completed-sale, or robust listing value remains `unknown` rather than becoming zero. Opening PV reads the service's already-published market snapshots and never starts a collector.
