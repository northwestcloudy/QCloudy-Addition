...
QCloudy_Addition is a client-only Fabric mod focused on readable SkyBlock maps, compact objective HUDs, client-side visual helpers, pet information, inventory quality-of-life tools, and opt-in party/chat utilities. The mod is bilingual, English-first, and keeps Hypixel-provided names in their original form. The current source tree is the unpublished `0.3.10-alpha7` development snapshot for Minecraft 26.1.2 only. The current public testing build remains Beta 0.3.10 for Minecraft 26.1.2 and 26.2; the latest stable release remains 0.3.9.
...
- [Changelog (current source 0.3.10-alpha7; public Beta 0.3.10 below)](CHANGELOG.md)
...
- **Dungeon Player Quick View** — when the exact Dungeon Finder message reports that a new player joined the dungeon group, QCA analyzes only that newcomer and prints a colored chat card. It shows Catacombs level, total and per-run Secrets, all five class levels plus a one-decimal Class Average, the advertised floor's run count and fastest completion, four armor slots, Withered Blade/Terminator and Golden Dragon/Ender Dragon presence, and Magical Power. The class row follows Odin's compact visual order—Archer/Berserk/Healer/Mage/Tank—as class-colored numeric levels separated by `/`; hovering a number identifies the class and shows exact XP. To avoid permanent floor `Missing` results before queueing, QCA reads only the local player's own already-open Party Finder listing, proven by the bottom-row delist control, and caches that advertised floor; it does not inspect another group's profile. Recognized armor, weapons, and pets use native item-style hover details, and missing or private fields say `Missing`. The measured top and bottom separators use the active Minecraft font's normal and bold line-glyph widths to match endpoints exactly. QCA never checks class conflicts and never kicks automatically; only clicking the red underlined action runs `/party kick <player>`. The feature has an independent Dungeons setting.
