...
The primary `/v1/pv/{target}` response contains identity, selectable Profile descriptors, independent source metadata, and non-Dungeon section payloads. Museum and Garden are supplementary fixed endpoints so their six/twelve-hour caches stay independent from the one-hour complete-profiles cache; supplementary failure leaves the already loaded main snapshot usable. Server-side NBT decoding emits bounded item summaries rather than Base64, and Misc/Farming plus otherwise unclassified fields use the same bounded projection budget. Museum validates membership and returns only the queried UUID's member. `ProfileViewerScreen` uses QCA surfaces/cards, explicit loading/error/private/missing/partial/stale states, a profile selector, and separate directly draggable sidebar/content `VerticalScrollbar` instances.
