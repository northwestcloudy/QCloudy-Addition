...
`DungeonJoinParser` accepts the exact Dungeon Finder newcomer message as the only admission target and returns a structured player/class/level event. It separately recognizes the exact queue-confirmation line and ordinary party joins for lifecycle bookkeeping. `DungeonQuickViewManager` is reached only after an authoritative Mod API Location confirms SkyBlock and the independent default-on Dungeon Quick View setting is enabled; it deduplicates repeat admission lines for two seconds, cancels requests across session changes, and analyzes only the captured newcomer. `DungeonPartyFinderFloorTracker` accepts only an already-open Party Finder container whose bottom row proves the local player's own listing through the delist control. The exact queue confirmation starts a new listing generation but retains an advertised floor already proven by that own listing, because Hypixel normally closes the GUI before the next newcomer message; the request can therefore continue to send `?floor=F…/M…`. Queue confirmation still clears all roster and DUPE authority. The first complete own-listing roster in the new generation freezes players already present as trusted, and later ordinary/manual joins are trusted too. Those players are never profiled, rejected, kicked, or printed by admission handling; their classes exist only as DUPE context. Exact Party Finder newcomers remain pending until their own decision passes. Exact leave/removal/offline/disband/local-kick/new-party/delist messages, rejected admissions, listing/floor changes, world/server changes, and disconnects remove the applicable state. A pending newcomer that arrives before the initial GUI roster remains pending and cannot be folded into the trusted baseline. The queued scoreboard remains a request/display fallback, but it is never an automatic-action authority.
