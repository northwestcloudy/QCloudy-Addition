...
## Player Profile Viewer and QCloudy market service

`ProfileCommands` registers Brigadier literal `/pv` for the documented user input `//pv`, plus ordinary client literal `qpv`; it never claims ordinary `/pv`. Each root has an optional single `StringArgumentType.word()` player-name/UUID argument and otherwise reads the local game profile name. They construct `ProfileViewerScreen` on the client thread and never call `sendCommand` or `sendChat`.

`QcaApiClient` is a fixed-route Java HTTP client. Its fixed data origin is `https://api.qcloudy.net/` on port 443, redirects are disabled, connect/request timeouts are five/fifteen seconds, and each response is streamed through a four-MiB hard cap before UTF-8 parsing. Arbitrary URLs, user info, fragments, changed response URIs, non-HTTPS schemes, and alternate ports are rejected. `ProfileJsonParser` accepts schema version 1 only, bounds strings/profile counts, ignores additive unknown sections, and has no Dungeon enum. `ProfileService` validates names/UUIDs locally, uses monotonically increasing generations to cancel superseded work, maps bounded error envelopes, and stores successful snapshots in a process-only `SessionProfileCache`. The local entry expires at the earliest relevant server boundary and never later than ten minutes.

The primary `/v1/pv/{target}` response contains identity, selectable Profile descriptors, independent source metadata, and non-Dungeon section payloads. Museum and Garden are supplementary fixed endpoints so their six/twelve-hour caches stay independent from the one-hour complete-profiles cache; supplementary failure leaves the already loaded main snapshot usable. Server-side NBT decoding emits bounded item summaries rather than Base64. Every top-level section receives an independent node/byte projection budget, so a large early section cannot replace Skills, Minions, or later sections with a shared-budget sentinel. Museum validates membership and returns only the queried UUID's member. `ProfileViewerScreen` renders a persistent identity/Profile header, icon navigation, metric cards, progress bars, semantic groups, and slot-based item grids in QCA's dark/cyan language. It exposes explicit loading/error/private/missing/partial/truncated/stale states and does not recursively print arbitrary JSON, internal IDs, raw timestamps, or projection sentinels as the primary UI.

The deployable FastAPI service lives in `backend/qcloudy-api`. `Settings` loads `QCA_HYPIXEL_API_KEY` only from the server environment. `HypixelUpstream` has named methods rather than a generic proxy and attaches `API-Key` only to player/profiles/Museum/Garden calls; Bazaar, active auctions, ended auctions, public resources, and Mojang name resolution receive no key. `CacheStore` is Redis-first with a process-memory fallback, request coalescing, stale-on-technical-error behavior, and distributed collector locks. Successful private/missing data is a new cache value and therefore replaces older visible data.

`MarketManager` refreshes Bazaar every 60 seconds, active AH every 120 seconds, ended auctions every 30 seconds, and static resources every six hours; public item resources have a 14-day technical cache. Active AH performs page 0, bounded-parallel remaining pages, and a final page-0 source-version check; any missing/drifting page rejects the complete cycle, so publication is atomic. Item identity uses bounded NBT parsing and a stable variant fingerprint. Active BIN aggregation exposes lowest BIN and the median of the lowest up to five listings. Ended auctions are deduplicated in SQLite by auction ID, kept for approximately 30 days, and record a coverage gap if the collector misses the official 60-second window. A complete market source has all required published components; `partial` preserves usable values while at least one component is stale or unavailable. Opening PV or the Planner reads published snapshots and does not start or accelerate a collector.

`POST /v1/market/tooltip-prices` accepts a bounded batch of item ID, optional actual `variantKey`, and quantity. AH output is rendered in this exact order: optional `NPC Sell Price`, current clean/base-variant `Low. BIN Price`, 72-hour time-weighted clean-LBIN `3 Day Avg. Price`, and actual held-variant `Item NW Value`. Clean LBIN samples are stored separately in SQLite; any invalid coverage gap breaks the continuous window, and the average remains unavailable during the initial 72 hours rather than substituting an ended-auction statistic. Bazaar output is optional `NPC Sell Price`, `BZ Sell Price` from `instantSellPrice` (coins received immediately), then `BZ Buy Price` from `instantBuyPrice` (coins paid immediately). Each quote carries `status`, `unitCoins`, and `totalCoins`; missing/non-positive/non-finite values are omitted. Quantity greater than one displays total first and the grey `(unit each)` afterwards, while quantity one has no suffix. The client renders complete comma-grouped Coin values without K/M abbreviations, uses measured columns with orange labels and cyan values, inserts no generic price header, coalesces identical in-flight requests, caches a successful tooltip result for at most 60 seconds, and cancels obsolete requests when the hovered item changes or the screen closes.
...
| Player enters `//pv [player or UUID]` or `/qpv [player or UUID]` | Opens the read-only screen and, unless a valid process cache entry is usable, performs one bounded HTTPS main-profile request to fixed `api.qcloudy.net` | No Minecraft server payload; explicit user action |
| Player changes SkyBlock Profile or retries the PV screen | Performs the corresponding bounded HTTPS main-profile request, subject to the process cache and request-generation cancellation | No Minecraft server payload; explicit user action |
| Player opens uncached Museum or Garden in PV | Performs one bounded HTTPS request to that fixed supplementary endpoint; the main snapshot remains usable if it fails | No Minecraft server payload; explicit user action |
| Player hovers a decoded item in PV whose price is not cached | Performs one bounded, coalesced HTTPS tooltip-price batch request to the fixed QCloudy endpoint; changing hover or closing PV cancels obsolete work | No Minecraft server payload; explicit UI action |
