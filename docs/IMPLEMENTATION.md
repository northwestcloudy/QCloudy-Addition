...
The scoreboard admission state separately distinguishes confirmed Hub/queue context from empty/partial, departed, and active-dungeon context. Strong in-run evidence wins over stale queue rows. QCA rereads the live vanilla scoreboard before asking for the final PartyInfo snapshot and again inside the final guard, rather than allowing the periodic cache alone to authorize an action. Chat-cached classes never authorize DUPE. A DUPE result requires a complete own-listing GUI roster read after the admission, with every applicable frozen existing name and the target name resolved through live Tab UUIDs into the exact PartyInfo membership. If any rule is confirmed failed, the manager requests a second final PartyInfo snapshot and re-reads the GUI roster after that final request; both sources must be at most two seconds old and inside their hard deadlines. It then emits one failure header and every still-confirmed failed rule. Immediately adjacent in the same client-thread decision turn, it may call `sendCommand("party kick <validated newcomer>")` once. The final guard rechecks authoritative SkyBlock state, the same session/world, queue and cached ownership-proven listing generations/floor, unchanged policy revision and values, the exact second PartyInfo ticket, local `LEADER` role, target UUID/Tab membership, current target class when DUPE is enabled, and a unique action key. A cleared/replaced listing context, Entrance or scoreboard-only floor, partial/departed/active-dungeon state, PartyInfo/GUI timeout or staleness, no party, lost leadership, absent target, identity/evidence uncertainty, config change, session/world change, duplicate delivery, or missing connection prevents the automatic command. The client never prints PASS, “no kick command was sent,” or “kick sent,” never counts down or warns Party Chat, and never claims the server actually removed the player.

The source FastAPI service exposes one bounded Dungeon response. It resolves the name, loads player and SkyBlock Profiles concurrently, and treats Profile selection as certain only when exactly one Profile is marked selected. With zero selected Profiles it displays the most recently saved visible Profile; with multiple selected Profiles it displays the most recently saved among those marked selected, but both cases leave all admission evidence UNKNOWN. It projects only Catacombs/class XP, the requested floor's completions/fastest time, total Secrets and the legacy displayed all-run average, Magical Power, four armor slots, selected weapon presence, and two dragon pets. Bounded NBT decoding retains formatted name and at most 80 lore lines for native client hover construction. Player/Profile sources are fresh for two minutes with a ten-minute stale ceiling used only on technical failure. Private/missing data stays unavailable. Requirements evidence reports its source freshness and coverage; notably, the current average uses account Secrets over selected-Profile completions, so its scopes do not align and the evidence deliberately returns `SCOPE_MISMATCH`/UNKNOWN. Magical Power uses the selected Profile's `highest_magical_power`, a historical maximum rather than a live current value. The server key remains environment-only and the endpoint is not a generic proxy. These Alpha 8 backend changes are source-only at this point: production has not been deployed, and neither the evidence response nor automatic action has been authenticated-live tested on Hypixel. A production response without evidence version 1 is display-only/UNKNOWN and cannot authorize an automatic kick.
