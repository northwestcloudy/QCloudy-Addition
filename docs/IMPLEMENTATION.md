...
`DungeonQuickViewMessage` builds one colored multi-line chat component. Catacombs attaches an exact-XP `SHOW_TEXT` hover. The class line follows Odin's compact presentation: Archer, Berserk, Healer, Mage, and Tank are rendered as class-colored one-decimal levels separated by `/`, with the class name and exact XP in each value's hover and no average level. Armor, recognized weapons, and recognized pets are converted into local ItemStacks and attached through `HoverEvent.ShowItem(ItemStackTemplate.fromNonEmptyStack(...))`, preserving Minecraft's native item-tooltip arrangement. The header measures the styled title with the active `Font`; the lower separator dynamically combines normal and bold line glyphs so its measured endpoint exactly matches the upper separator when the bold advance supplies the otherwise unreachable remainder. The final red, bold, underlined `ClickEvent.RunCommand("/party kick <validated name>")` is the only kick path. No result, missing field, class combination, or timer can execute it automatically.
