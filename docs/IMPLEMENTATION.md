...
`HypixelSessionTracker` is the single environment authority. The official Hypixel Mod API `ClientboundHelloPacket` confirms the network independently of the server-list address; `ClientboundLocationPacket` authoritatively selects SkyBlock versus another game and supplies server, mode, map, and lobby context. A world change clears instance fields until the next Location event, while disconnect clears the whole session. Address text is never an activation gate. While Location is pending, an exact Hypixel proxy brand plus a strict `SKYBLOCK` sidebar title and a structural Profile/Purse/location/Dungeon/Rift row can temporarily allow passive rendering, but never command-sending features. Scoreboard and Tab parsing then enrich the confirmed session with Profile, subarea, Dungeon, Rift, task, powder, and pet state; loose substring presence can no longer establish server identity. Separately, `DungeonPartyAuthorityTracker` uses the same official implementation to send `ServerboundPartyInfoPacket` only for an active admission decision and consume `ClientboundPartyInfoPacket`; its fresh UUID-to-`PartyRole` map is a final action authority, never a substitute for Location or listing ownership.

### 1.2 Always-on Release notification

This is lifecycle infrastructure, not a configurable gameplay feature, so it has no `ModConfig` value and no feature card. `ReleaseBuildInfo` loads the build resource containing the QCA channel, version, Minecraft target, and positive Release-baseline sequence; Beta 0.3.10 embeds the currently published stable baseline `1`. The client entrypoint registers the world-join listener for every channel, but Alpha returns from the local gate before scheduling a task or constructing an HTTP request. For Beta and Release, the atomic process guard in `ReleaseUpdateChecker` permits only one scheduled check for the lifetime of the client process. The first join schedules the check after five seconds; the network work runs away from the render/client thread.
...
The screen layer derives inputs, buttons, lists, detail viewports, and click targets from shared layout records. Shard results and metadata have separate scroll state, rate controls are bottom-docked, narrow Planner/Settings layouts reflow before collision, and Fusion Lines use a canvas taller than the viewport when needed. Setting rows add hit targets only while intersecting the visible scissor rectangle. The main catalog, feature settings, compatibility report, and party whitelist share a browser-style vertical scrollbar: the thumb can be grabbed without jumping, dragging remains captured outside the track, clicking the track moves one page, and wheel input is held still while the thumb is captured.
...
- Boolean rows with a secondary editor display a localized inline right-click cue beside the visual switch; the cue does not create a second switch or alter the click targets.
...
- **Inputs:** the directly owned `Player.fishing` hook when available; physical local fishing-rod use; already-loaded Fishing Hooks within a bounded association radius; entities inside the selected hook's bounding box expanded by four blocks; received ArmorStand metadata after vanilla applies it; and exact received name/visibility state.
- **Implementation:** directly owned water or lava hooks always win. `FishingHookResolver` records the hook IDs already present when the local player physically uses a fishing rod, then accepts only a newly loaded local-owned or ownerless candidate during the next 40 ticks; an explicit other-player owner is rejected. Packet-driven observation can associate that candidate without consuming the tick-based window when no candidate exists. The same resolver classifies another physical rod use while the direct or fallback hook is still active as reeling, not as a new cast. `ClientPacketListenerMixin` calls `FishingBiteAlert` at the tail of the received entity-data handler, after vanilla has applied the complete update; the existing tick scan remains a fallback. Both paths still require an invisible ArmorStand with a visible custom name exactly equal to `!!!` inside the selected hook's four-block box and share `FishingBiteSession`, keyed by hook entity ID. The supplied MP3 is converted before packaging to `assets/qcloudy_addition/sounds/fishing/ciallo.ogg` and registered by `sounds.json`.
...
- **Fast Party Commands:** this parent switch defaults off. Its independent children for Warp, All Invite, Transfer, Kick, Coordinates, Promote, Stream, Dungeon, and Kuudra default on. The local player's own `!` messages use one separate Boolean. Non-self `!warp` uses one audience and every other non-self `!` command shares another: None, Party members, Friends, Guild members, or Guild members and friends. Because parsing accepts only Party Chat, those audiences narrow already-proven party members; an independent case-insensitive whitelist is the only explicit non-self override. Friend membership is proven by the complete paginated `/fl` transaction already used by Party Auto Accept. Guild membership is stored only after a `/g members` transaction contains both Online and Offline headings, consistent section/total counts, and its closing rule; partial/online-only output remains unknown. Neither list nor the state-changing `/g onlinemode` toggle is requested automatically. `!warp`/`!w` and `!allinvite`/`!all`/`!allinv` retain shared cooldowns of five seconds and two seconds respectively.
...
- **Private utilities:** Party Auto Accept remains a separately configured local friend/whitelist check. Its account-scoped friend cache accepts only structured `/friend list` rows whose visible name, `/viewprofile` UUID action, and profile hover name agree. Each verified row from the currently visible page is immediately usable; only a complete ordered `1..N` traversal may replace the authoritative snapshot and prune former friends. Streamed rows must also exactly match `<name> is currently offline` or `<name> is in <location>`, so interleaved public, guild, or party chat cannot contaminate the roster. Normal and bold special friends remain distinct, and pre-schema-3 caches are invalidated for a fresh structured verification. Private-message Party Request is off by default and sends `party invite <sender>` only for an exact allowed received keyword. Quick Private `!p` is off by default; local `//invited <player>`, `//invited by <player>`, and `//i <player>` send `msg <player> !p`.
...
- **Prices:** `ShardPriceService` delegates asynchronously to `ShardBazaarService`, which accepts only the schema-v1 Shard response from the fixed `QcaApiClient` origin. `instant_buy` maps to Hypixel Bazaar `quick_status.buyPrice`; `instant_sell` maps to `quick_status.sellPrice`. Values must be finite and positive, product IDs must be bounded `SHARD_*` identifiers, and a local result is cached for at most ten minutes without crossing server metadata. No SkyHanni/Skyblocker/Firmament reflection or hard dependency remains. Missing, stale-expired, malformed, or unavailable values fail closed while Ironman and rate-only planning remain independent.
...
- `config/qcloudy_addition.json`: language, feature settings, the default-off Dungeon admission master and 14 independent F1–F7/M1–M7 rule sets, per-HUD appearance/position/scale, remembered pet details, Hunting resources/Chapter/Benefactor/Safari Belt state, collected Fairy Soul keys, and Shard Planner settings/rates/graph positions. The old `autumecloudyaddition.json` is read once for migration. Schema 30 creates rules format 1 with every rule disabled and no Entrance entry.
...
## Dungeon Player Quick View and QCloudy service
...
`DungeonJoinParser` accepts the exact Dungeon Finder newcomer message as the only admission target and returns a structured player/class/level event. It separately recognizes the exact queue-confirmation line and ordinary party joins for lifecycle bookkeeping. `DungeonQuickViewManager` is reached only after an authoritative Mod API Location confirms SkyBlock and the independent default-on Dungeon Quick View setting is enabled; it deduplicates repeat admission lines for two seconds, cancels requests across session changes, and analyzes only the captured newcomer. An `AbstractContainerScreen` click-boundary mixin observes only left-button `PICKUP` input. At the click, `DungeonPartyFinderFloorTracker` requires the exact `Group Builder` title, the clicked emerald-block `Confirm Group` with `Click to confirm!` lore, and named `Select Dungeon Type`/`Select Floor` entries with `Currently Selected:` lore. Catacombs, Master Mode, Entrance, and I–VII are normalized to `F…`, `M…`, or `E`; Search Settings, malformed proof, and unrelated clicks fail closed. The result is an inert 15-second candidate until the exact queue-success line promotes it. That confirmation begins a new listing generation and emits a localized detected-floor or `Missing` chat status. The tracker separately accepts an already-open Party Finder container whose bottom row proves the local player's own listing through the delist control; this can confirm or replace the cached floor and supplies roster authority. Queue confirmation still clears all roster and DUPE authority. The first complete own-listing roster in the new generation freezes players already present as trusted, and later ordinary/manual joins are trusted too. Those players are never profiled, rejected, kicked, or printed by admission handling; their classes exist only as DUPE context. Exact Party Finder newcomers remain pending until their own decision passes. Exact leave/removal/offline/disband/local-kick/new-party/delist messages, rejected admissions, listing/floor changes, world/server changes, and disconnects remove the applicable state and Group Builder candidate. A pending newcomer that arrives before the initial GUI roster remains pending and cannot be folded into the trusted baseline. The queued scoreboard remains a request/display fallback, but it is never an automatic-action authority.

`DungeonQuickViewService` validates player names and floor IDs, coalesces an identical in-flight request, and keeps a successful process-only result for Profile display for 60 seconds. An enabled automatic admission decision bypasses that completed cache and uses a new acquisition (or the identical request already in flight); its response has a ten-second local monotonic decision window and a source-epoch sanity check matching the backend's longest fresh-source contract. `QcaApiClient` sends one fixed-route request to `/v1/dungeons/quick-view/{target}` with the optional floor query. Its HTTPS origin, no-redirect policy, five-second connection timeout, full fifteen-second headers-plus-body deadline, and non-blocking four-MiB-bounded body subscriber are shared with the still-independent Shard request. A never-ending body is cancelled and completes as a timeout. `DungeonQuickViewSnapshot` accepts only bounded quick-view schema 1. Optional requirements evidence versions 1 and 2 are bound to the query name, resolved UUID, selected Profile ID/certainty, requested and returned F1–F7/M1–M7 floor, freshness, and source coverage. Version 1 remains transition-compatible but its average-Secrets value is forced to UNKNOWN; version 2 proves the matching selected-Profile scope. A missing, malformed, unsupported, stale, identity-mismatched, floor-mismatched, or uncertain evidence block becomes UNKNOWN; it is never coerced to a failing zero. Equipment and pets likewise retain present/confirmed-absent/unknown states.

`ModConfig.PartyFinderAutoKick` is separate from the Profile display and defaults off. Enabling it opens `DungeonAutoKickConfirmScreen`. Its format-version-1 map contains exactly F1–F7 and M1–M7, each with a separately allocated `DungeonFloorRequirements`; normalization drops unknown/Entrance entries, repairs missing or aliased objects by deep copy, clamps invalid numeric values, and preserves a disabled rule's saved value. Four numeric toggle/value pairs cover minimum exact-floor completions, maximum fastest completion milliseconds, minimum average Secrets, and minimum historical-highest Magical Power. Five Boolean toggles cover duplicate-class rejection and required Wither Blade, Terminator, Golden Dragon, and Ender Dragon. Every rule defaults off. The settings screen selects one floor at a time, so editing F7 cannot alter M7 or any other floor. Entrance has no rule object or policy fallback. Any master, rule-toggle, threshold, or parent Quick View edit increments an in-memory policy revision and cancels active admissions; later restoring the old value cannot revive an asynchronous decision created under the earlier revision.

`DungeonRequirementEvaluator` is pure and processes enabled rules in a stable nine-item order. Minimum comparisons use `>=`, the fastest-time maximum uses `<=`, equipment/pets fail only on confirmed absence, and duplicate class fails only after complete roster/identity/class reconciliation finds a named conflict. A confirmed Mage newcomer omits average Secrets from the finding list entirely; an unavailable newcomer class makes that enabled rule UNKNOWN. No enabled rules means the ordinary Profile path. A result is FAIL when at least one rule is confirmed failed, otherwise UNKNOWN when at least one rule lacks authoritative evidence, otherwise PASS. Thus a confirmed failure may coexist with a different UNKNOWN rule; UNKNOWN itself never becomes a failure.

`DungeonQuickViewMessage` builds one colored multi-line Profile component. Catacombs attaches an exact-XP `SHOW_TEXT` hover. Average Secrets uses exactly one decimal on the compact Profile card; the snapshot and evaluator keep the original full-precision value, and requirement-failure details use round-trip-safe formatting so close actual/threshold values remain distinguishable. The class line follows Odin's compact presentation: Archer, Berserk, Healer, Mage, and Tank are rendered as class-colored one-decimal levels separated by `/`, with the class name and exact XP in each value's hover, followed by a one-decimal Class Average. Any missing class keeps the average `Missing`. Armor, recognized weapons, and recognized pets are converted into local ItemStacks and attached through `HoverEvent.ShowItem(ItemStackTemplate.fromNonEmptyStack(...))`, preserving Minecraft's native item-tooltip arrangement. PASS emits only this Profile; it adds no PASS/no-kick line. UNKNOWN emits the same Profile followed by all localized unknown reasons and never sends a command. A request/transport failure with no snapshot emits the bounded unavailable notice and cannot enter evaluation. Profile cards retain the red, bold, underlined manual `/party kick <validated player>` click action.

For a policy-bearing own listing, the manager freezes the join event, queue/listing generations, exact floor policy and policy revision, the names observed in the immediately pre-join own-listing GUI roster, membership identity, and authority session, then requests an initial fresh PartyInfo packet. QCA serializes its own overlapping admissions and gives every refresh a unique monotonic ticket. PartyInfo itself has no request ID and `HypixelModAPI` is shared by other mods, so required mixins wrap the entire shared `sendPacket` call: every successful PartyInfo send is serialized and appended to a FIFO for the current physical connection as either an exact QCA `(authority session, ticket)` owner or a foreign marker. A PartyInfo packet or error consumes exactly the oldest marker at the shared API handler entry, before registered third-party callbacks can throw or return; only an exact active QCA owner can create a snapshot. The packet constructor carries its network-decode `nanoTime` into later client-thread handling, exact snapshots are bounded, and missing/unmatched/errored/foreign/old-world/evicted traffic cannot be replaced by a newer snapshot. Each ticket records its own physical dispatch `nanoTime`; its three-second response deadline begins there rather than at enqueue, while the admission separately caps unsent queue waiting at ten seconds. Finishing, replacing, or cancelling an admission deletes only its pending unsent tickets—an active marker remains in the connection ledger until its response consumes that exact FIFO position. Ambiguous attribution poisons automatic authority until the physical connection changes, including across ordinary world changes.

The scoreboard admission state separately distinguishes confirmed Hub/queue context from empty/partial, departed, and active-dungeon context. Strong in-run evidence wins over stale queue rows. QCA rereads the live vanilla scoreboard before asking for the final PartyInfo snapshot and again inside the final guard, rather than allowing the periodic cache alone to authorize an action. Chat-cached classes never authorize DUPE. DUPE takes the frozen active QCA class roster excluding the current newcomer, takes fresh PartyInfo membership excluding that newcomer, and maps every frozen name through the live Tab UUID. Count mismatch, exact identity-set mismatch, missing identity, missing/malformed/conflicting/stale class, or unknown newcomer class returns a concrete DUPE UNKNOWN. Only DUPE stops; all other enabled rules continue, and any confirmed failure remains eligible for the independent final action guard. If any rule is confirmed failed, the manager requests a second final PartyInfo snapshot, emits every still-confirmed failed rule plus concrete errors, and immediately adjacent in the same client-thread decision turn may call `sendCommand("party kick <validated newcomer>")` once. The final guard rechecks authoritative SkyBlock state, the same session/world, queue and cached ownership-proven listing generations/floor, unchanged policy revision and values, the exact second PartyInfo ticket, local `LEADER` role, target UUID/Tab membership, and a unique action key. A cleared/replaced listing context, Entrance or scoreboard-only floor, partial/departed/active-dungeon state, PartyInfo timeout or staleness, no party, lost leadership, absent target, config change, session/world change, duplicate delivery, or missing connection prevents the automatic command. The client never prints PASS, “no kick command was sent,” or “kick sent,” never counts down or warns Party Chat, and never claims the server actually removed the player.

The source FastAPI service exposes one bounded Dungeon response. It resolves the name, loads player and SkyBlock Profiles concurrently, and treats Profile selection as certain only when exactly one Profile is marked selected. With zero selected Profiles it displays the most recently saved visible Profile; with multiple selected Profiles it displays the most recently saved among those marked selected, but both cases leave all admission evidence UNKNOWN. It projects only Catacombs/class XP, the requested floor's completions/fastest time, the selected Profile's Dungeon Secrets and explicit F1–F7/M1–M7 completion sum, historical-highest Magical Power, four armor slots, selected weapon presence, and two dragon pets. Entrance `0`, aggregate `total`, account achievements, and blood-mob compensations do not enter the average. Bounded NBT decoding retains formatted name and at most 80 lore lines for native client hover construction. Player/Profile sources are fresh for two minutes with a ten-minute stale ceiling used only on technical failure. Private/missing/malformed values and zero completed runs stay unavailable. Requirements evidence version 2 includes numerator, denominator, the `SELECTED_PROFILE_SECRETS_F1_F7_M1_M7_RUNS` scope, and completeness. The server key remains environment-only and the endpoint is not a generic proxy. These Alpha 9 backend changes are source-only until the production server is separately updated and restarted; neither the version-2 response nor automatic action has been authenticated-live tested on Hypixel. A version-1 production response keeps average Secrets UNKNOWN, and a response without supported evidence remains display-only/UNKNOWN.

`MarketManager` remains for the Shard Planner and fixed market API. It refreshes Bazaar every 60 seconds, active AH every 120 seconds, ended auctions every 30 seconds, and static resources every six hours; Dungeon requests do not start or accelerate collectors.
...
| Exact Dungeon Finder line announces a new group member while Dungeon Quick View is enabled | Performs one bounded, coalesced HTTPS quick-view request for that newcomer and prints/evaluates the local result | Automatic QCloudy HTTPS read; normally no Minecraft command |
| The own F1–F7/M1–M7 listing has an enabled admission policy | Sends one official `ServerboundPartyInfoPacket`; after a confirmed FAIL it sends one second final packet, for at most two fresh PartyInfo UUID/role snapshots per admission | Automatic authority refresh; not chat or a gameplay command |
| At least one rule is confirmed FAIL and every final guard still passes | Prints all confirmed failure reasons, then `sendCommand("party kick <validated newcomer>")` in the same client-thread decision turn | Yes; only after the separately confirmed, default-off master, an enabled exact-floor rule, fresh local-leader/target membership, and unchanged own-listing/session/policy guards |
| Player hovers a Dungeon Quick View statistic or item | Shows already-loaded local XP text or a native ItemStack tooltip | No network or server payload |
| Player clicks the underlined Dungeon Quick View kick action | `sendCommand("party kick <validated newcomer>")` through Minecraft's `RUN_COMMAND` chat click event | No; physical click required |
| Player opens the Shard Planner and its price snapshot loads | Performs one bounded asynchronous HTTPS Shard-price read from fixed `api.qcloudy.net`; no market collector is started | No Minecraft server payload; explicit user action |
...
| Enabled Fast Party Commands receives an allowed recognized Party Chat alias | `sendCommand` with the documented Party/Stream/`joininstance` payload | Yes, only after parent, child, self/audience/whitelist authorization, parser, completion, and cooldown checks |
