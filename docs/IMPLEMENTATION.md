...
`DungeonQuickViewMessage` builds one colored multi-line chat component. Catacombs and all five classes attach exact-XP `SHOW_TEXT` hovers; class names use native underlining. Armor, recognized weapons, and recognized pets are converted into local ItemStacks and attached through `HoverEvent.ShowItem(ItemStackTemplate.fromNonEmptyStack(...))`, preserving Minecraft's native item-tooltip arrangement. The header and lower separator use measured glyph widths and share endpoints within one separator glyph. The final red, bold, underlined `ClickEvent.RunCommand("/party kick <validated name>")` is the only kick path. No result, missing field, class combination, or timer can execute it automatically.
