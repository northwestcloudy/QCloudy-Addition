...
`DungeonQuickViewMessage` builds one colored multi-line chat component. Catacombs attaches an exact-XP `SHOW_TEXT` hover. The class line follows Odin's compact presentation: Archer, Berserk, Healer, Mage, and Tank are rendered as class-colored one-decimal levels separated by `/`, with the class name and exact XP in each value's hover, followed by a one-decimal Class Average. Any missing class keeps the average `Missing`. `DungeonPartyFinderFloorTracker` scans only an already-open Party Finder container, requires a bottom-row bookshelf delist control, chooses the bottom-row local-party head or the local leader's matching result, and parses paired Catacombs `Dungeon:`/`Floor:` lore. This cached advertised floor takes priority over the queued-scoreboard fallback when constructing the existing API request. Armor, recognized weapons, and recognized pets are converted into local ItemStacks and attached through `HoverEvent.ShowItem(ItemStackTemplate.fromNonEmptyStack(...))`, preserving Minecraft's native item-tooltip arrangement. The final red, bold, underlined click remains the only kick path.

The deployable FastAPI service exposes one bounded Dungeon response. It resolves the name, loads player and SkyBlock Profiles concurrently, selects the active or most recently saved visible Profile, and projects only Catacombs/class XP, the requested floor's completions/fastest time, total Secrets and all-run average, Magical Power, four armor slots, selected weapon presence, and two dragon pets. Bounded NBT decoding retains formatted name and at most 80 lore lines for native client hover construction. Player/Profile sources are fresh for two minutes with a ten-minute stale ceiling used only on technical failure. The server key remains environment-only and the endpoint is not a generic proxy.
...
| Exact Dungeon Finder line announces a new group member while Dungeon Quick View is enabled | Performs one bounded, coalesced HTTPS quick-view request for that newcomer and prints the local chat card | Automatic read-only display; no Minecraft server payload |
