...
## 统一设置与 HUD 控制——0.3.10-alpha8 开发快照
...
彩色聊天卡一次显示：Catacombs 等级、总 Secrets/所有地牢完成次数的平均 Secrets、五职业等级及一位小数 Class Average、当前发布楼层的完成次数与最快时间、四件护甲、Wither Blade/Terminator、Golden Dragon/Ender Dragon，以及 Magical Power。职业行采用类似 Odin 的紧凑样式，按 Archer/Berserk/Healer/Mage/Tank 顺序显示带职业颜色的等级数字，以 `/` 分隔；悬停每个数字会显示职业名称与精确 XP。楼层跟踪只接受同一 Party Finder 菜单中由取消发布控件证明属于本机的组队条目，并缓存其成对的 `Dungeon:`/`Floor:` lore；其他队伍与搜索筛选不能成为请求楼层。排队计分板只保留为展示/请求回退，但不能证明本机拥有发布条目，因此不能授权自动操作。识别到的护甲、武器和宠物使用原生物品悬停；确认没有时显示叉，来源私密、缺失或不完整时显示 `Missing`，不伪装成 0。
...
规则证据会绑定已解析 UUID、所选 Profile、请求/返回楼层、新鲜度与来源覆盖。当前卡片显示的平均 Secrets 使用“账号总 Secrets ÷ 所选 Profile 的地牢完成次数”，分子与分母范围不一致。因此源码后端把平均 Secrets 规则标为 `SCOPE_MISMATCH`；在获得范围一致的证据前，这条规则只能是 UNKNOWN。MP 条件使用所选 Profile 的历史 `highest_magical_power`，不是实时当前总值，因为这个来源没有可信的当前总值字段。

自动指令只能由提供 F1–F7/M1–M7 规则、已证明属于本机且保持同一 generation 的缓存发布条目上下文授权。QCA 会先通过 Hypixel 官方 Mod API 请求一次新鲜 PartyInfo；确认 FAIL 后再请求第二份最终快照，因此每名新成员最多两次。由于 PartyInfo 没有请求 ID 且 API 会被多个 Mod 共用，QCA 会按物理连接的 FIFO 顺序记录每次成功的 QCA/外部 PartyInfo 发送，只接受精确匹配 QCA 会话/票据的回包；缺失或无法确定归属都会安全停止。DUPE 还必须在最终请求后重新读取完整的自己的发布职业表，包括目标当前职业。发送前会重读实时计分板，并立即再次确认：本机 UUID 的角色仍为 `LEADER`、目标 UUID 仍与成员/Tab 身份一致，而且会话、世界、排队/发布 generation、楼层、policy revision 与数值、精确成员快照、目标、截止时间和单次动作键均未变化。Entrance、只靠计分板识别楼层、残缺/已离开/地牢内上下文、发布条目被清除/替换、PartyInfo/GUI 证据过旧、失去队长、目标离队、规则变化或会话/世界变化，都会阻止指令。

Alpha 8 的后端证据源码已在本仓库中，但尚未部署到生产环境；这一流程也尚未在登录 Hypixel 的实服中验证。若当前线上服务尚未返回规则证据，客户端会把已开启规则视为 UNKNOWN/仅展示，不能授权自动踢人。
