...
所有 QCA 自有设置界面都使用有边界的响应式布局。Shard 详情在独立滚动的信息区下方预留固定设置栏；Planner 控件在碰撞前重排；窄设置界面使用单列；Fusion Lines 扩展为可滚动画布。长文字只在真实可用范围内适配，裁剪区域外的行不会保留不可见点击范围。自身还带下级编辑页的布尔项会在可视化开关旁显示本地化“右键打开编辑”提示；左键切换开关，右键进入编辑页。
...
**功能：**开启后，QCA 优先使用直接归属于本地玩家、已经加载的 Fishing Hook。玩家真实使用钓竿后，还可以在有限的 40 tick 窗口内关联一根新加载但 owner 为空的鱼钩，使 Hypixel 岩浆钓鱼也能收到同样提示；抛竿前已存在的鱼钩和明确属于其他玩家的鱼钩都会被排除。选中鱼钩周围四格内，仅当存在“实体不可见、名称可见、收到的名称精确等于 `!!!`”的 ArmorStand 时，才播放一次提供的 Ciallo 提示音。客户端会在原版应用每次 ArmorStand 名称元数据后立即检测，并保留普通 client tick 扫描作为兜底。持续存在的标记无法通过任一路径重复播放，收杆也不会在 `!!!` 消失前重新打开播放门；只有确认的新抛竿或鱼钩消失才会为下一次提示重新待命。功能位于独立一级“钓鱼”分类，默认关闭，拥有独立的 0–100% 连续音量滑块，默认 64%。

**安全边界：**检测器只读取已加载客户端实体与客户端已经收到的实体元数据，并观察玩家真实使用钓竿的动作，不取消也不替换该动作。较大范围的鱼钩查找仅在短关联窗口内或已关联回退鱼钩仍加载时运行；空闲状态不会每 tick 扫描鱼钩。它只播放模组资源包内的 OGG，不收杆、抛竿、点击、转向、移动、切换物品，也不发送数据包、聊天或命令。
...
**市场价格：**QCA 从 `api.qcloudy.net` 读取自己经过转换和大小限制的快照；不要求安装其他客户端模组，也不读取 SkyHanni、Skyblocker 或 Firmament 的内部结构。打开 Planner 并加载价格时会进行一次有界异步 HTTPS 读取。Bazaar 获取成本使用 `buyPrice`（立即买入），出售/净值使用 `sellPrice`（立即卖出）。服务器每 60 秒刷新 Bazaar，技术故障时最多提供十分钟且明确标注的旧快照。完整来源表示所需的已发布市场组成均可用；`partial` 表示一个或多个组成已过时或不可用，但仍有部分可用价格。没有可靠价格的单个物品保持 `unknown`，未知、畸形或非正数值都会省略，绝不改成 0。Ironman 仍然完全不使用 Bazaar；服务不可用时，全部离线、速率、配方、详情、线路和仓库功能继续可用。
...
自动入队处理拥有单独总开关，默认关闭；从关闭改为开启时会先弹出明确的确认页。关闭 Dungeon Quick View 会同时停止请求/Profile 与入队流程。自动操作总开关关闭或当前层全部规则关闭时，仍照常输出普通 Profile；Entrance 同样始终只显示 Profile。配置 schema 30 使用规则格式版本 1，只保存 14 份完全独立的设置：F1–F7 与 M1–M7。Entrance 不存在规则，一层的设置绝不会继承、回退到或修改另一层。
...
3. 最快完成时间上限。
4. 平均 Secrets 下限。
5. 历史最高 Magical Power 下限。
6. 必须拥有 Wither Blade。
7. 必须拥有 Terminator。
8. 必须拥有 Golden Dragon。
9. 必须拥有 Ender Dragon。

四项数值规则分别有启用开关和值；关闭规则不会清空已填数值。五项布尔规则直接开关。迁移后全部规则默认关闭，因此某层没有开启任何规则时仍照常输出 Profile。已开启规则按 AND 关系判断；实际值等于最低/最高边界时算通过。

精确排队确认后，第一次完整且归属明确的自己的发布名单会冻结可信旧队员；普通/手动加入队伍的玩家也可信。QCA 不会检查、输出、拒绝或踢出这些玩家，只保留他们的职业供之后 DUPE 使用。精确 Party Finder 新人会保持待判定，直到自己的判定通过；离队、被移除、解散、本机被踢/加入其他队、取消发布、发布/楼层替换、换会话/世界/服务器或判定失败，都会清理对应状态。

最终输出只有三种：

- **PASS：**只输出 Profile，不显示 PASS，也不显示“未发送踢人指令”。
- **UNKNOWN：**输出 Profile，随后列出所有无法确认的原因，绝不踢人。缺失/私密字段、证据过旧或不匹配、Profile 选择不确定、武器/宠物覆盖不完整、队伍职业不完整、证据版本不支持或 PartyInfo 不可用，全部安全视为 UNKNOWN，而不是 0 或 FAIL。若网络/传输失败导致完全没有快照，则只显示有界的“无法取得”提示，同样不能踢人。
- **FAIL：**先输出一行失败标题与全部已经确认不满足的规则；最终操作保护仍全部通过时，紧接着在同一个客户端判定回合发送 `party kick <已校验新成员>`。文字不会声称服务器已经接受指令，也没有倒计时、Party Chat 警告、PASS 行、“未发送踢人指令”或“已发送踢人指令”行。同一次判定中只要存在已确认失败，FAIL 会优先于其他 UNKNOWN 项；UNKNOWN 不会被错误列成失败。

### 11.3 请求、证据与最终操作保护

整张卡只向固定 `https://api.qcloudy.net` 发出一次有界异步 HTTPS 请求。禁止跳转，连接超时五秒；十五秒截止时间覆盖响应头与完整正文，非阻塞订阅器将响应限制为 4 MiB，永不结束的正文会被取消。客户端不含 Hypixel API Key。相同进行中请求会合并，成功响应只用于 Profile 展示并缓存 60 秒；已开启的自动判定会绕过这份完成缓存，并另有十秒本地证据窗口。离开会话时取消未完成工作。源码中的后端并行加载 player 与 Profiles，各自缓存两分钟；仅在技术故障时允许最多十分钟旧值。私密与缺失仍明确显示，不会被旧成功数据掩盖。

规则证据会绑定已解析 UUID、唯一 selected Profile、请求/返回楼层、新鲜度与来源覆盖。证据版本 2 使用该 Profile 的 `member.dungeons.secrets`，除以同一 Profile 明确的 F1–F7 与 M1–M7 完成次数之和。Entrance、`total`、账号 Achievement 与 Odin 的 blood-mob 补偿都不参与；数据缺失/格式异常或分母为 0 时保持 UNKNOWN。精确确认新人为 Mage 时完整跳过这条规则；新人职业未知时这条规则为 UNKNOWN。部署过渡期客户端接受版本 1 和 2，但会把版本 1 平均值强制降级为 UNKNOWN。MP 条件使用所选 Profile 的历史 `highest_magical_power`，不是实时当前总值。

自动指令只能由提供 F1–F7/M1–M7 规则、已证明属于本机且保持同一 generation 的缓存发布条目上下文授权。QCA 会先通过 Hypixel 官方 Mod API 请求一次新鲜 PartyInfo；确认 FAIL 后再请求第二份最终快照，因此每名新成员最多两次。由于 PartyInfo 没有请求 ID 且 API 会被多个 Mod 共用，QCA 会按物理连接的 FIFO 顺序记录每次成功的 QCA/外部 PartyInfo 发送，只接受精确匹配 QCA 会话/票据的回包；缺失或无法确定归属都会安全停止。多个入队判定重叠时，票据在单飞队列中等待不会提前消耗三秒回包窗口；队列等待仍有独立上限，已取消且尚未发送的票据会立即移除。DUPE 从 QCA 冻结有效名单与新鲜 PartyInfo 中同时排除当前新人，再用实时 Tab UUID 精确对齐身份。人数不一致、身份集合不一致、名字无法映射、队员职业缺失/异常/冲突/过期，或新人职业未知时，只终止 DUPE 并输出具体错误，其他规则继续；其他规则确认失败时仍可进入最终踢人保护。发送前会重读实时计分板，并立即再次确认：本机 UUID 的角色仍为 `LEADER`、目标 UUID 仍与成员/Tab 身份一致，而且会话、世界、排队/发布 generation、楼层、policy revision 与数值、目标、截止时间和单次动作键均未变化。Entrance、只靠计分板识别楼层、残缺/已离开/地牢内上下文、发布条目被清除/替换、权威证据过旧、失去队长、目标离队、规则变化或会话/世界变化，都会阻止指令。

Alpha 9 的版本 2 后端证据源码已在本仓库中，但 Mod 构建不会自动部署，仍需单独更新并重启生产后端；这一流程也尚未在登录 Hypixel 的实服中验证。线上仍返回版本 1 时，平均 Secrets 保持 UNKNOWN；没有受支持规则证据时，全部已开启后端规则都只能 UNKNOWN/展示。
