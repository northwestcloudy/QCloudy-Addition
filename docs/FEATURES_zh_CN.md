...
## 统一设置与 HUD 控制——0.3.10-alpha9 开发快照
...
自动指令只能由提供 F1–F7/M1–M7 规则、已证明属于本机且保持同一 generation 的缓存发布条目上下文授权。QCA 会先通过 Hypixel 官方 Mod API 请求一次新鲜 PartyInfo；确认 FAIL 后再请求第二份最终快照，因此每名新成员最多两次。由于 PartyInfo 没有请求 ID 且 API 会被多个 Mod 共用，QCA 会按物理连接的 FIFO 顺序记录每次成功的 QCA/外部 PartyInfo 发送，只接受精确匹配 QCA 会话/票据的回包；缺失或无法确定归属都会安全停止。DUPE 从 QCA 冻结有效名单与新鲜 PartyInfo 中同时排除当前新人，再用实时 Tab UUID 精确对齐身份。人数不一致、身份集合不一致、名字无法映射、队员职业缺失/异常/冲突/过期，或新人职业未知时，只终止 DUPE 并输出具体错误，其他规则继续；其他规则确认失败时仍可进入最终踢人保护。发送前会重读实时计分板，并立即再次确认：本机 UUID 的角色仍为 `LEADER`、目标 UUID 仍与成员/Tab 身份一致，而且会话、世界、排队/发布 generation、楼层、policy revision 与数值、目标、截止时间和单次动作键均未变化。Entrance、只靠计分板识别楼层、残缺/已离开/地牢内上下文、发布条目被清除/替换、权威证据过旧、失去队长、目标离队、规则变化或会话/世界变化，都会阻止指令。
