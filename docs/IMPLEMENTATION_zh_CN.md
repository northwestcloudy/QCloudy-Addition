...
本文跟踪仅适配 Minecraft 26.1.2 的未公开 `0.3.10-alpha9` 源码快照，逐项说明每个功能的用途、读取的客户端信息、实现方式、应呈现的效果、默认状态，以及是否会产生对外操作。当前公开测试版仍为 Beta `0.3.10`，最新稳定版仍为 Release `0.3.9`。
...
- 二级设置页的所有布尔值统一绘制为 QCA 青色开关，不再显示“开启/关闭”文字；左键切换。父项关闭时，子开关变暗但保留真实位置。只有确实拥有下级编辑器的开关响应右键；Party Finder 入队要求由本页唯一持有总开关，右键可在关闭状态下预配置各楼层规则。
