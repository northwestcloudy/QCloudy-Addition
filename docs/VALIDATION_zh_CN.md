...
Beta 0.3.10 将完整 Alpha 37 实现晋级为公开测试版本，只额外修改发布通道/版本文档，并补上此前遗漏的 Mod Menu“源代码”链接。它包含稳定版 Release 0.3.9 之后完成的全部玩家可见变化。

## 最终产物

- `release/QCloudy_Addition-0.3.10+26.1.2-Beta.jar` — 3,631,856 字节 — `61f9ddd64905d4d64e75183c035d8de018e762a81e20aa9691e42da7811c9733`
- `release/QCloudy_Addition-0.3.10+26.1.2-Beta-sources.jar` — 3,102,801 字节 — `161bfe36ea4253d18d141736d441be1274fabdb9a38165852004508e8c719ded`
- `release/QCloudy_Addition-0.3.10+26.2-Beta.jar` — 3,631,850 字节 — `47cdd80e3599f8eb587a3e7825e230f7b210edc250d00cfe8ab49cd364528f18`
- `release/QCloudy_Addition-0.3.10+26.2-Beta-sources.jar` — 3,102,787 字节 — `f9c0537ff92b686b502f229fe4c5b878032455351911eae3ffc0b5859c87cd2f`
...
- 展开元数据版本为 `0.3.10-beta+26.1.2` 与 `0.3.10-beta+26.2`。
- 打包发布元数据声明通道 `Beta`、核心版本 `0.3.10`、精确 Minecraft 目标与稳定 Release 基线序号 `1`。
...
- `release/QCloudy_Addition-0.3.9+26.1.2-Alpha-37.jar` — 3,631,814 字节 — `cb8c923d4e2d333b5001dfec068ab6a4cd40c24afa81731f7847b725b0f7a765`
- `release/QCloudy_Addition-0.3.9+26.1.2-Alpha-37-sources.jar` — 3,102,759 字节 — `30da6186f5effec7dde02865ad65959cc0f8c33e88f5835a8aacbd4c81a50215`

Alpha 37 修复“好友组队邀请自动接受”的好友表同步，并让 QCA 设置界面的滚动条可以直接操作。`/friend list` 标题、结构化好友行与底部分隔线现在可以作为多条聊天组件依次到达；每条可见名称、`/viewprofile` UUID 点击动作和资料悬浮名称一致的好友会立即可用，只有按顺序完整读取全部页面后才允许清理权威缓存。独立逐行消息还必须严格符合离线或所在位置状态，因此夹入的公屏、公会或组队聊天不会进入好友表。好友缓存升级为 schema 3，旧的空缓存或过度信任缓存必须重新完成一次结构化验证。主设置目录、功能设置页、兼容性报告和组队白名单现在共用浏览器式滚动条，支持抓住滑块、拖出轨道继续移动、点击轨道翻页与更宽的鼠标命中范围。

本工作区已验证：

- Minecraft 26.1.2 独立运行 51 个测试套件、293 项测试，0 failure、0 error、0 skipped。
- Minecraft 26.2 另行运行 51 个测试套件、293 项测试，0 failure、0 error、0 skipped；该目标只用于兼容验证，不发布也不交付 26.2 Alpha 产物。
- 好友表回归测试覆盖用户提供的第一页玩家名与在线/离线状态、聚合/逐行聊天组件、当前页即时授权、普通/粗体特别好友分类、完整顺序快照清理、结构化公屏/公会近似消息拒绝、断线重置与 schema 2 失效。
- 滚动条回归测试覆盖比例/最小滑块、首尾精确映射、抓取偏移不跳位、轨道外拖动、轨道翻页及首尾钳制、隐藏状态无响应、抓取期间禁止滚轮改变位置、释放后恢复滚轮，以及主目录独立槽位。
- 已打包可运行 JAR 声明 `0.3.9-alpha.37+26.1.2`，要求 Java 25，目标为 Minecraft 26.1.2，并包含已编译的 `VerticalScrollbar`、`FriendRoster` 与 schema 3 存储实现。
- 可运行 JAR 与 Sources JAR 和目标专属 `build/26.1.2/libs/` 输出逐字节相同，并全部通过 JDK 25 `jar --validate` 与 `unzip -t`。
- 归档检查在两个交付文件中均发现 0 个重复条目，也没有带冲突后缀的 class 条目。
- Alpha 37 是不公开发布的 Alpha 构建；以上检查不会把它变成 Beta 或 Release，也不授权创建 GitHub Release、Tag 或 Modrinth 版本。

尚未覆盖的实服回归边界：

- 自动测试模拟了 Hypixel 带点击/悬浮信息的结构化好友行，但本地构建未在已登录的 Hypixel 会话中独立启动。请执行 `/fl`，确认当前设置模式下可由一个普通好友和一个粗体特别好友触发自动接受，并浏览完全部页面一次，确认旧条目会被清理。
- 请在完整模组环境中抓住四个新滚动条的滑块，按住后把鼠标移出轨道，并分别点击滑块上方/下方。打开菜单后的前 180ms 仍存在原有的视觉缩放与输入坐标不完全一致，因此动画结束后也需要重复验证。

---

# QCloudy_Addition 0.3.9 Alpha 36 验证
