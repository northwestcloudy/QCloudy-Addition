...
- Building the mod does not deploy the backend. The separately handed-off Alpha 9 patch ZIP must be uploaded/extracted at `/opt/qcloudy-api` and the service restarted; until then a version-1 response keeps average Secrets UNKNOWN, and a response without supported evidence keeps backend rules UNKNOWN/display-only. Exact BaoTa steps and rollback are recorded in `backend/qcloudy-api/deploy/README.md`.
- No authenticated live Hypixel run has exercised the full real own-listing → newcomer → PartyInfo → output → optional command sequence. Local tests do not prove live menu/chat wording, leadership races, server acceptance, successful removal, or absence of every future false positive.
- This section does not claim a Minecraft 26.2 Alpha artifact, game-directory installation, production deployment, GitHub Release, or Modrinth publication. Public Beta 0.3.10 and stable Release/update baseline 0.3.9 remain unchanged.
...
- Java `25.0.4` ran `clean test build prepareRelease` independently for Minecraft 26.1.2 and 26.2.
...
- This audit does not claim a fresh authenticated Hypixel regression for the upgraded Bonzo message, all `/friend list` pages, Party Auto Accept, or live Party commands.
- It does not claim a fresh HMCL visual check of the Official Page action, a real Mod Menu inspection of all three links, full provider-modpack testing, or every GUI scale/OS/resource-pack combination.
- Beta is a public testing channel and remains a pre-release; it is not the stable Release and is not Hypixel endorsement.
...
The combined instance warnings/errors concerned reference-mod refmaps/resources, an optional ModernUI class, missing BabyzombieAddons custom-disc files, SkyHanni/Skyblocker remote-repository requests, unauthenticated profile/Realms activity, and SkyHanni 7.41.0 rejecting the current NEU-repository constants `HUNTING_FORTUNE` and `FISHING_NET`. They were not thrown from QCA and did not stop client/resource/sound initialization. QCA has no Firmament runtime dependency. Item timestamps and cursor memory remain controlled only by their own QCA feature switches and are not disabled merely because Firmament is installed.
