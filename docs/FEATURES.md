...
## Unified settings and HUD control — 0.3.10-alpha2 development snapshot
...
`//pv`, `//pv <player or UUID>`, `/qpv`, and `/qpv <player or UUID>` open the same read-only QCA screen. Omitting the target uses the local Minecraft player; the ordinary `/pv` root is intentionally left untouched for compatibility. The selected SkyBlock profile can be changed inside the persistent header. Icon-based navigation replaces long category labels where an unambiguous Minecraft item exists, while stat cards, progress bars, semantic groups, and slot-based item grids replace the former key/value dump. The viewer has no settings card and no Dungeon/automatic party-profile component.

The screen organises the transformed response into Overview, Gear, Accessories, Pets, Inventory/Ender Chest/Storage/Wardrobe/Vault, Skills, Slayer, Mining, Minions, Bestiary, Collections, Crimson Isle/Kuudra/Dojo/Trophy Fish, Rift, Miscellaneous/Farming, Museum, Garden, and Market/Net Worth. Private, absent, partial, truncated, stale, and service-failure states are shown explicitly. Decoded item containers become visual slot grids; internal IDs, raw timestamps, projection sentinels, encoded NBT, and arbitrary JSON recursion are not used as the presentation. Each section has its own server projection budget, so a large inventory cannot consume the allowance for Skills, Minions, or later sections.
...
The server collects Bazaar approximately every 60 seconds, a complete version-consistent active-AH cycle approximately every two minutes, and ended auctions every 30 seconds. An AH cycle publishes atomically only if every page and the final page-zero check share one source version. The tooltip request is bounded, coalesced, and cached for at most 60 seconds on the client; moving to another item or closing the viewer cancels obsolete work. It only reads already-published snapshots and never starts or accelerates a collection cycle.

AH tooltip rows have one fixed order: optional `NPC Sell Price`, `Low. BIN Price` for the clean/base variant's current lowest BIN listing, `3 Day Avg. Price` for the time-weighted clean-LBIN history, and `Item NW Value` for the actual held variant. The three-day value requires 72 continuous hours of valid clean-LBIN coverage and is omitted before that initial window is complete; it is not an ended-auction median. Bazaar rows are optional `NPC Sell Price`, `BZ Sell Price` for the coins received from an immediate sale, then `BZ Buy Price` for the coins paid for an immediate purchase. No generic price heading or separator is inserted. Missing, invalid, inapplicable, or unsafe rows are omitted rather than rendered as zero. For a quantity above one, the first price is the full stack total and the grey parenthesised suffix is the per-item value; quantity one has no suffix. Coin amounts use complete comma-grouped numbers rather than K/M abbreviations. Orange labels and cyan values are laid out as measured columns rather than padded with spaces.
