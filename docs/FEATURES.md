...
## Unified settings and HUD control — 0.3.10-alpha12 development snapshot
...
**What it does:** while enabled, QCA prefers the local player's directly owned, already-loaded Fishing Hook. After a physical local rod use, it can also associate a newly loaded ownerless hook during a bounded 40-tick window so Hypixel lava fishing receives the same cue. Hooks present before the cast and hooks owned by another player are rejected. An invisible, visible-named ArmorStand whose exact received name is `!!!` inside the selected hook's four-block box triggers the supplied Ciallo cue once for that cast. The marker is checked immediately after vanilla applies each received ArmorStand metadata update, with the ordinary client-tick scan retained as a fallback. Persistent markers cannot replay the sound through either path, and using the rod to reel does not re-arm the cue while that marker is disappearing; only a confirmed new cast or removed hook arms the next cast. The feature defaults off, lives in the top-level Fishing category under the **Bite Alerts** subgroup, and has its own continuous 0–100% volume slider at the project-wide 64% default.

**Safety:** the detector reads loaded client entities and already-received entity metadata, and observes the player's physical rod use without cancelling or replacing it. The broader hook lookup runs only during the short association window or while that fallback hook remains loaded; idle gameplay does not scan for hooks every tick. It plays one bundled local OGG and does not reel, cast, click, rotate, move, select an item, send a packet, chat message, or command.
...
One colored chat card shows Catacombs level, selected-Profile Secrets and its F1–F7/M1–M7 per-run average, all five class levels plus a one-decimal Class Average, the advertised floor's completion count and fastest time, four armor slots, Wither Blade/Terminator and Golden Dragon/Ender Dragon presence, and historical-highest Magical Power. The compact Odin-style class row shows class-colored numeric levels in Archer/Berserk/Healer/Mage/Tank order, separated by `/`; each number identifies its class and exact XP on hover. At the exact left-click on the emerald-block `Confirm Group`, the floor tracker reads `Select Dungeon Type` and `Select Floor` from that same `Group Builder`; the result remains inert for at most 15 seconds and becomes current only after the exact queue-success chat line. QCA then immediately prints the detected `F/M/E` floor, or `Missing`, for live diagnosis. The tracker also accepts the local player's own Party Finder entry when the same menu exposes the delist control and caches its paired `Dungeon:`/`Floor:` lore. Other parties, Search Settings, malformed menus, and unrelated clicks cannot become the request floor. The queued scoreboard remains a display/request fallback, but never proves ownership and therefore cannot authorize automatic removal. Recognized armor, weapon, and pet entries construct Minecraft's native item hover; absent equipment shows a cross, while unavailable/private/incomplete data says `Missing` rather than zero.
...
5. Minimum historical-highest Magical Power.
...
Players present in the first complete own-listing roster after the exact queue confirmation are a trusted baseline. Ordinary/manual party joins are trusted too. QCA never profiles, prints, rejects, or kicks those players; their latest own-listing classes are retained only for later DUPE checks. Exact Party Finder newcomers remain pending until their own admission passes. Departures, removals, disbanding, local kick/new-party messages, delisting, listing/floor replacement, session/world/server changes, and rejected admissions remove the applicable state.
...
Requirements evidence is tied to the resolved UUID, exactly selected Profile, requested and returned floor, freshness, and source coverage. Evidence version 2 calculates average Secrets as that selected Profile's `member.dungeons.secrets` divided by the sum of its explicit F1–F7 and M1–M7 completions. Entrance, `total`, account achievements, and Odin's blood-mob adjustment are excluded. Missing/malformed values or a zero denominator are UNKNOWN. A confirmed Mage newcomer skips this rule completely; an unavailable newcomer class makes it UNKNOWN. During deployment transition the client accepts evidence versions 1 and 2, but forces version-1 average evidence to UNKNOWN. The MP criterion uses the selected Profile's historical `highest_magical_power`, not a live-current total.

An automatic command is authorized only from the same cached, ownership-proven listing context that supplied an F1–F7 or M1–M7 policy. QCA first requests fresh PartyInfo through the official Hypixel Mod API; after a confirmed FAIL it requests a second final snapshot, for at most two requests per admission. Because PartyInfo has no request ID and the API is shared, QCA records every successful QCA/foreign PartyInfo send in physical-connection FIFO order and accepts only the exact QCA session/ticket; missing or ambiguous attribution fails closed. Overlapping admissions wait in that single-flight FIFO without spending their three-second response window before physical dispatch, retain a separate bounded queue wait, and discard obsolete tickets that have not yet been sent. DUPE compares the frozen active QCA roster and fresh PartyInfo membership after excluding the current newcomer from both, then resolves exact identities through live Tab UUIDs. Count mismatch, identity-set mismatch, missing identity, missing/malformed/conflicting/stale member class, or missing newcomer class stops only DUPE and prints the concrete error; every other rule continues. Confirmed non-DUPE failures may still authorize the final kick. Immediately before sending, QCA rereads the live scoreboard and rechecks that the local UUID has `LEADER` role, the resolved target UUID is still a member/Tab identity, and the session, world, queue/listing generation, floor, policy revision and values, target, deadlines, and one-shot action key are unchanged. Entrance, scoreboard-only floor detection, partial/departed/active-dungeon context, a cleared/replaced listing, stale authority, lost leadership, an absent target, a changed policy, or a session/world change prevents the command.

The Alpha 9 version-2 backend evidence source is present in this repository but is not deployed by the mod build, and this path has not been authenticated-live tested on Hypixel. Production must be updated and restarted separately. A service that still returns version 1 keeps average Secrets UNKNOWN; a service without supported requirements evidence keeps all enabled backend rules UNKNOWN/display-only.
