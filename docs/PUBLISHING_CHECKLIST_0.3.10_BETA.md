# Beta 0.3.10 publication checklist
...
- Validate the exact four 0.3.10 Beta archives with `jar --validate` and `unzip -t`.
- Confirm playable metadata declares `0.3.10-beta+<minecraft>`, client-only, Java 25, and the exact target/Fabric API dependency.
...
- Confirm the packaged release properties declare `Beta`, version core `0.3.10`, the exact Minecraft target, and baseline sequence `1`.
...
- Create tag `v0.3.10-beta` on that commit.
- Create `QCloudy_Addition 0.3.10 Beta` as a GitHub **Pre-release**.
- Use `docs/GITHUB_RELEASE_0.3.10_BETA.md` as the main body.
...
- Use `docs/MODRINTH_RELEASE_0.3.10_BETA.md` as changelog.
...
- Prepare matching Beta 0.3.10 download/changelog content separately from the source commit.
