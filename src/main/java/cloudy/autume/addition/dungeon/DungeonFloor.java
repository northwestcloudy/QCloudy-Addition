...
public record DungeonFloor(String id) {
    ...
    private static Observation observe(Iterable<String> lines) {
        ...
        if (!queued || tier == null) return new Observation(queued, master, Optional.empty());
        String normalized = tier.toUpperCase(Locale.ROOT);
        if (normalized.contains("ENTRANCE")) {
            return new Observation(true, master, Optional.of(new DungeonFloor("E")));
        }
        java.util.regex.Matcher number = java.util.regex.Pattern
                .compile("(?:FLOOR\\s*)?([IVX]+|[1-7])$").matcher(normalized);
        if (!number.find()) return new Observation(true, master, Optional.empty());
        int value = roman(number.group(1));
        if (value < 1 || value > 7) return new Observation(true, master, Optional.empty());
        return new Observation(true, master,
    ...
    private record Observation(boolean queued, boolean master, Optional<DungeonFloor> floor) { }
}
