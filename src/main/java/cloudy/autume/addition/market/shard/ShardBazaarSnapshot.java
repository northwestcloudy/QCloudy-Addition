...
/** Schema-v1 Shard-only Bazaar prices returned by the QCloudy backend. */
public final class ShardBazaarSnapshot {
    public static final int SCHEMA_VERSION = 1;
    private static final int MAX_PRICES = 4096;

    private final String source;
    private final MarketSourceMetadata metadata;
    private final ShardBazaarSide side;
    private final Map<String, Double> prices;

    public ShardBazaarSnapshot(int schemaVersion, String source,
                               MarketSourceMetadata metadata, ShardBazaarSide side,
    ...
    public MarketSourceMetadata metadata() { return metadata; }
    public ShardBazaarSide side() { return side; }
    public Map<String, Double> prices() { return prices; }
    public boolean stale() { return metadata.stale(); }
    ...
    public static ShardBazaarSnapshot parse(String json) {
        try {
            ...
            if (schema != SCHEMA_VERSION) {
                throw new MarketDataException(MarketDataException.Code.UNSUPPORTED_SCHEMA,
            ...
            MarketSourceStatus status = MarketSourceStatus.parse(
            ...
            MarketSourceMetadata metadata = new MarketSourceMetadata(status,
                    instant(rawMetadata, "fetchedAt"), instant(rawMetadata, "expiresAt"),
                    instant(rawMetadata, "staleUntil"), instant(rawMetadata, "nextRefreshAt"),
                    sourceVersion);
            ...
            throw new MarketDataException(MarketDataException.Code.INVALID_RESPONSE,
                    "The Shard Bazaar service returned invalid data.", Duration.ZERO, 0, exception);
        }
    }
